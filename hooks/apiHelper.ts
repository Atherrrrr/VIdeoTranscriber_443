...
export async function postFetcher(url: string, data: PostData, isRaw = false) {
  const res = await _post(url, data);
  await _handleError(res);
  return isRaw ? res : res.json();
}

export async function putFetcher(url: string, data: PostData) {
  const res = await _put(url, data);
  await _handleError(res);
  return res.json();
}

export async function getFetcher(url: string, isRaw: boolean = false) {
  const res = await _get(url);
  await _handleError(res);
  return isRaw ? res : res.json();
}

export async function deleteFetcher(url: string, isRaw = true) {
